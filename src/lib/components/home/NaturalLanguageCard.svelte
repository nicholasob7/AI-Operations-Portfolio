<script lang="ts">
	...
	type Props = {
		...
		openPrecision: () => void;
		closePrecision: () => void;
		suppressFloatingClose?: boolean;
		onFloatingCloseChange?: (visible: boolean) => void;
	};
	...
	let {
		...
		openPrecision,
		closePrecision,
		suppressFloatingClose = false,
		onFloatingCloseChange
	}: Props = $props();
	...
	$effect(() => {
		onFloatingCloseChange?.(showFloatingClose);
	});
...
<article id="about-head" class="project-card project-card-language" tabindex="-1">
	...
	<p class="project-outcome">
		I use disciplined semantic control to drive stable AI behavior. I turn complex technical situations into
...
{#if showPrecision && showFloatingClose && !suppressFloatingClose}
